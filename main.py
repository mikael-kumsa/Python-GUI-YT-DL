...
canvas=Canvas(root,width=600,height=100, bg="#17202A", highlightthickness= 0)
canvas.grid(row=0,column=0)

Banner=ImageTk.PhotoImage(Image.open("PYTD.png"))
canvas.create_image(0,0,image=Banner,anchor=NW)

tab1=Frame(root,width=800,height=350, bg="#17202A")
...
urlbox=Entry(tab1,width=60,font=("Arial",13))
urlbox.grid(row=0,column=0,columnspan=2,pady=20)
urlbox.focus_force()

label1=Label(tab1,text="",font=("Arial ",12),relief="solid", bg="#17202A")


canvas=Canvas(tab1,width=180,height=120,bg="grey")
canvas.grid(row=3,column=1,rowspan=6,pady=6)


Directory="Phoenix Youtube Downloader"
Dir_Name="C:/"
path=os.path.join(Dir_Name,Directory)
try:
  os.mkdir(path)
  os.mkdir("C:/Phoenix Youtube Downloader/Video")
  os.mkdir("C:/Phoenix Youtube Downloader/Audio")
except OSError as error:
  pass

length_video=Label(tab1,text="",font=("Arial Bold",13), bg="#17202A", fg="white")
length_video.grid(row=3,column=0,sticky=W,pady=6)

author=Label(tab1,text="",font=("Arial Bold",13), bg="#17202A", fg="white")
author.grid(row=4,column=0,sticky=W)

views=Label(tab1,text="",font=("Arial Bold",13), bg="#17202A", fg="white")
views.grid(row=5,column=0,sticky=W)

reso=Label(tab1,text="",font=("Arial Bold",13), bg="#17202A", fg="white")
...
def get():
  ...
  yt=YouTube(url)
  title=yt.title
  label1.grid(row=2,column=0,columnspan=2,pady=10, padx=10)
  label1.config(text=title)
  #DISPLAYING THUMBNAIL
  u=urlopen(yt.thumbnail_url).read()
  im=Image.open(BytesIO(u))
  resize=im.resize((180,120), Image.ANTIALIAS)
...
def download():
  ...
  if VIDEO==TRUE:
    ...
    os.chdir("C:/Phoenix Youtube Downloader/Video")
    try:
  ...
  if AUDIO==TRUE:
    ...
    my_progress.stop()
    label2.grid(row=9,column=1)
    os.chdir("C:/Phoenix Youtube Downloader/Audio")
    try:
